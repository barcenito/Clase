		...
		while (true) {
			try {
				...
				case 2: {
					...
					User usuario = dao1.getUserById(id, session);
					if (usuario != null)
				...
				case 3: {
					for(User usuario :dao1.getAllUsers(session)) {
						System.out.println(usuario);
					}
				...
				case 4: {
					...
					int id = Integer.valueOf(input.readLine());
					User user = dao1.getUserById(id, session);
					if (user == null) {
				...
				case 5: {
					...
					dao1.deleteUserById(id,session);
